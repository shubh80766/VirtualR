            ...
            {pricingOptions.map((option,index)=>(
                <div key={index} className="w-full sm:w-1/2 lg:w-1/3 p-2">
                    <div className="p-10 border border-neutral-700 rounded-xl">
                        <p className="">
                            {option.title}
                            {option.title==="Pro" && <span className="bg-gradient-to-t from-orange-500 to-red-500 text-transparent bg-clip-text text-xl mb-4 ml-2">(Most Popular)</span>}
                            </p>
                            <p className="mb-8">
                            ...
                            <ul>
                                {option.features.map((feature,map)=>(
